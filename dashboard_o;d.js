...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch thong ke data
      const response = await fetch("https://pos.doanquochoa.name.vn/api/thong-ke", {
        method: "POST",
        headers: {
      ...
      const nhapHangResponse = await fetch(
        `https://pos.doanquochoa.name.vn/api/sale-nhap-hang-by-ch?${nhapHangParams}`
      );
      const nhapHangResult = await nhapHangResponse.json();

      // Sort by SoPN in ascending order
      const sortedNhapHangData = nhapHangResult.sort((a, b) => {
        const soPN_A = parseInt(a.SoPN) || 0;
        const soPN_B = parseInt(b.SoPN) || 0;
        return soPN_A - soPN_B;
      });

      setNhapHangData(sortedNhapHangData);

      // Fetch ton kho data
      const tonKhoParams = new URLSearchParams({
        MaCH: filters.dsMaCH[0], // Use first selected store for now
      ...
      // Fetch shift data
      // Sử dụng đúng API với query string
      ...
      const shiftUrl = `https://pos.doanquochoa.name.vn/api/sale-ket-ca?${shiftParams.toString()}`;
      const shiftResponse = await fetch(shiftUrl);
      const shiftResult = await shiftResponse.json();

      // shiftResult đã là [{ Ngay, Ca: [...] }]
      setShiftData(Array.isArray(shiftResult) ? shiftResult : []);

      const allBanhKemData = [];
      // Fetch doanh số bánh kem data
      for (const storeId of filters.dsMaCH) {
        try {
          const banhKemResponse = await fetch(
            "https://pos.doanquochoa.name.vn/api/doansobanhkem",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (banhKemResponse.ok) {
            const banhKemResult = await banhKemResponse.json();

            if (banhKemResult.success && banhKemResult.data) {
              allBanhKemData.push(...banhKemResult.data);
            }
          }
        } catch (error) {
          console.error(
            `Lỗi tải dữ liệu bánh kem cho cửa hàng ${storeId}:`,
            error
          );
        }
      }

      setBanhKemData(allBanhKemData);

      // Fetch wholesale cake data
      const allWholesaleData = [];
      for (const storeId of filters.dsMaCH) {
        try {
          const wholesaleResponse = await fetch(
            "https://pos.doanquochoa.name.vn/api/cake-wholesale-summary-detail",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (wholesaleResponse.ok) {
            const wholesaleResult = await wholesaleResponse.json();
            if (Array.isArray(wholesaleResult)) {
              allWholesaleData.push(...wholesaleResult);
            }
          }
        } catch (error) {
          console.error(
            `Lỗi tải dữ liệu bánh kem đặt cho cửa hàng ${storeId}:`,
            error
          );
        }
      }

      setWholesaleData(allWholesaleData);

      // Fetch inventory check data
      const kiemKeResponse = await fetch(
        "https://pos.doanquochoa.name.vn/api/phieu-kiem-ke"
      );
      if (kiemKeResponse.ok) {
        const kiemKeResult = await kiemKeResponse.json();

        // Filter by date range
        const filteredKiemKeData = kiemKeResult.filter((phieu) => {
        ...
        setKiemKeData(filteredKiemKeData);
      } else {
        setKiemKeData([]);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!loadingStores && filters.dsMaCH.length > 0) {
      fetchData();
    }
  }, [loadingStores]);
  ...
  const exportTonKho = async () => {
    ...
    khoHangData.forEach((item, index) => {
          ...
          <div className="tables-section">
            ...
            <div className="table-card">
              <div className="table-header">
                <div className="table-title">
                  <FiPackage />
                  <h3>Chi Tiết Bán Bánh Kem</h3>
                </div>
                <div className="table-actions">
                  <button
                    className="table-action"
                    onClick={exportBanhKem}
                    disabled={banhKemData.length === 0}
                  >
                    <FiDownload />
                    Xuất Excel
                  </button>
            ...
            <div className="table-card">
              <div className="table-header">
                <div className="table-title">
                  <FiPackage />
                  <h3>Chi Tiết Bánh Kem Đặt</h3>
                </div>
                <div className="table-actions">
                  <button
                    className="table-action"
                    onClick={exportWholesale}
                    disabled={wholesaleData.length === 0}
                  >
                    <FiDownload />
                    Xuất Excel
                  </button>
